...
CREATE MATERIALIZED VIEW public.view_all_organisms_info AS
	...
	FROM public.organism AS o
	
	JOIN public.species AS s
	    ON s.id = o.species_id
	
	JOIN public.sampling_area AS sa
	    ON sa.id = o.sampling_site_id
	
	JOIN public.habitat AS h
	    ON h.id = sa.habitat_id
	
	JOIN public.location AS l
	    ON l.id = sa.location_id
	
	JOIN public.country AS c
	    ON c.id = l.country_id
	
	CROSS JOIN public.property AS p
	
	JOIN public.trait AS tr
	    ON tr.id = p.trait_id
	
	LEFT JOIN public.organism_property AS op
	    ON op.organism_id = o.id
...
--
-- Create indexes for the core tables: -
--

CREATE INDEX IF NOT EXISTS idx_organism_species_id
ON public.organism(species_id);

CREATE INDEX IF NOT EXISTS idx_organism_sampling_site_id
ON public.organism(sampling_site_id);

CREATE INDEX IF NOT EXISTS idx_sampling_area_location_id
ON public.sampling_area(location_id);

CREATE INDEX IF NOT EXISTS idx_sampling_area_habitat_id
ON public.sampling_area(habitat_id);

CREATE INDEX IF NOT EXISTS idx_location_country_id
ON public.location(country_id);

CREATE INDEX IF NOT EXISTS idx_property_trait_id
ON public.property(trait_id);

CREATE INDEX IF NOT EXISTS idx_organism_property_organism_property
ON public.organism_property(organism_id, property_id);
